...
def page_config():
    ...
    if config:
        return render_template('config.html',
            server=config['server'], ad_ip=config['ad_ip'] or '',
            base_dn=config['base_dn'], username=config['username'],
            ou_usuarios=config['ou_usuarios'] or 'CN=Users',
            ou_computadores=config['ou_computadores'] or '')
    return render_template('config.html', server='', ad_ip='', base_dn='',
        username='', ou_usuarios='CN=Users', ou_computadores='')

@config_ad_bp.route('/api/config')
def get_config():
    conn = get_db_connection()
    config = conn.execute('SELECT server, ad_ip, base_dn, username, ou_usuarios, ou_computadores FROM config_ad WHERE id=1').fetchone()
    conn.close()
    if config:
...
def salvar_config():
    ...
    conn = get_db_connection()
    conn.execute(
        'REPLACE INTO config_ad (id, server, ad_ip, base_dn, username, password, ou_usuarios, ou_computadores) VALUES (1, ?, ?, ?, ?, ?, ?, ?)',
        (dados.get('server'), dados.get('ad_ip'), dados.get('base_dn'),
         dados.get('username'), dados.get('password'),
         dados.get('ou_usuarios'), dados.get('ou_computadores'))
    )
    conn.commit()
    conn.close()
